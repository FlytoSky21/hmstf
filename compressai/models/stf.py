...
class SymmetricalTransFormer(CompressionModel):
    def __init__(self,
        ...
        self.class_features = int(embed_dim * 2 ** (self.num_layers - 1))
        ...
        self.norm = norm_layer(self.class_features)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Linear(self.class_features, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x):
        ...
        y_class = y_hat
        y_class = self.norm(y_class)
        y_class = self.avgpool(y_class.transpose(1, 2))
        y_class = torch.flatten(y_class, 1)
        y_class = self.head(y_class)
        for i in range(self.num_layers):
